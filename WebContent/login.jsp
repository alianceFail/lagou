    	...
    	<input type="hidden" id="resubmitToken" value="" />		
		 <div class="login_box">
        	...
        	<form id="loginForm" action="index.html">
				<input type="text" id="email" name="account" value="" tabindex="1" placeholder="请输入登录邮箱地址" />
			  	<input type="password" id="password" name="pass" tabindex="2" placeholder="请输入密码" />
	...
	//验证表单
	 	$("#loginForm").validate({
	    	...
	    	submitHandler:function(form){
	            ...
	            $.ajax({
	            	type:'POST',
	            	data:{account:account,pass:pass,autoLogin:remember, callback:callback, authType:authType, signature:signature, timestamp:timestamp},
	            	url:'logine',
	            	dataType:'text'
	            }).done(function(data) {
					if(data=="index"){
					...
					else if(data=="index01")
					{
						window.location.href="index01.jsp";
					}
					else{
						$('#beError').text("账号或密码出错！！").show();
						}
