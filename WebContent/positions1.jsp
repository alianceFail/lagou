            ...
            <div class="content">
            	<dl class="company_center_content">
                    <dt>
                        <h1>
                            <em></em>
                           已下线职位 <span>（共<i style="color:#fff;font-style:normal" id="positionNumber">${countjob1}</i>个）</span>                        </h1>
                    </dt>
                    <dd>
                    		                    	<form id="query1" name="query1">
	                    		<input type="hidden" value="Publish" name="type">
			                	<ul class="reset my_jobs">
			                	<c:forEach var="list1" items="${list1}">
				                <li data-id="149594">
		                                    <h3>
		                                        <a target="_blank" title="positionName">${list1.positionName}</a> 
		                                        <span>${list1.workAddress}</span>
		                                        						                        		                                    </h3>
		                                    		                                  		<span class="receivedResumeNo"><a href="unHandleResumes.html?positionId=149594">应聘简历（1）</a></span>
		                                  			                                    <div>${list1.jobNature}/ ${list1.salaryMin}k-${list1.salaryMax}k / ${list1.workYear}/ ${list1.education}</div>
		                                    		                                    				                                    <div class="c9">发布时间：<fmt:formatDate value="${list1.date}" pattern="yyyy-MM-dd " /></div>
			                                    		                                    		                                    		                                    <div class="links">
		                                    			                                       	<a class="job_refresh" href="javascript:void(0)">刷新<span>每个职位7天内只能刷新一次</span></a>
		                                       			                                       	<a target="_blank" class="job_edit" href="toupdatejob?id=${list1.id}">编辑</a>
		                                       	<a  href="updateshangxian?id=${list1.id}">上线</a>                      
		                                        <a  class="job_del" href="del?id=${list1.id}">删除</a>
		                                    </div>
		                          </li>
		                              </c:forEach>      		                                
